...
class YtNoti:
    ...
    async def callback(self, video: Video) -> None:
        ...
        if video.channel.id == 'UCPCTEN8OWHdJGxfMggbyGGg':
    ...
    async def parse_youtube(self) -> None:
        ...
        _log.trace('Started youtube parsing, cheking: %s, count of videos found: %s', self.channel_ids,  len(self.video_history.videos))
        ...
        while self.running:
            ...
            for cid in self.channel_ids:
                ...
                if videos:
                    _log.trace('%s last video: %s', videos[0].channel.name, videos[0].url)
