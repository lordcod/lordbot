...
class TicketAllowedRolesDeleteDropDown(nextcord.ui.StringSelect):
    async def __init__(self, guild: nextcord.Guild, approved_roles: Optional[list]) -> None:
        ...
        super().__init__(placeholder='Select the allowed roles to delete', options=options[:25], max_values=min(25, len(options)), row=0, disabled=disabled)

    async def callback(self, interaction: nextcord.Interaction) -> None:
        gdb = GuildDateBases(interaction.guild_id)
        tickets: TicketsPayload = await gdb.get('tickets')
        ticket_data = tickets[self.view.message_id]
        ticket_data['approved_roles'] = list(set(ticket_data.get('approved_roles', [])) - set(map(int, self.values)))
        await gdb.set_on_json('tickets', self.view.message_id, ticket_data)

        view = await TicketAllowedRolesView(interaction.guild, self.view.message_id)
...
class TicketAllowedRolesDropDown(nextcord.ui.RoleSelect):
    async def __init__(self, guild: nextcord.Guild) -> None:
        super().__init__(placeholder='Select the allowed roles to add', max_values=25, row=1)
    ...
    async def callback(self, interaction: nextcord.Interaction) -> None:
        ...
        for role in self.values.roles:
            ...
            ticket_data['approved_roles'] = list(set(ticket_data.get('approved_roles', [])) | set(self.values.ids))
            await gdb.set_on_json('tickets', self.view.message_id, ticket_data)
...
class TicketAllowedRolesView(ViewOptionItem):
    label = 'Ticket Allowed Roles'
    description = 'Specify the roles that will be able to create tickets.'
